...
class GraphExtractor:
    ...
    def __init__(self, epsilon, min_road_dist,district,year):
        self.segment_simplifier = DouglasPeucker(epsilon)
        self.min_road_dist = min_road_dist
        self.district = district
        self.year =year

    def extract(self, skeleton, mbr, target_path,district,year):
        ...
        status_matrix = self.identify_node_pixels(binary_skeleton,district,year)
        sys.stdout.write("done.\n")
        sys.stdout.flush()

        sys.stdout.write("Detecting road network component... ")
        nodes, segments = self.detect_rn_component(status_matrix,district,year)
        sys.stdout.write("done.\n")
        sys.stdout.flush()

        sys.stdout.write("Constructing and saving road network... ")
        sys.stdout.flush()
        self.construct_undirected_rn(nodes, segments, target_path, nb_rows, xscale, yscale, mbr,district,year)
        sys.stdout.write("done.\n")
        sys.stdout.flush()

    def identify_node_pixels(self, skeleton,district,year):
        """
        22 23 08 09 10
        21 07 00 01 11
        20 06 -1 02 12
        ...
        for i, j in zip(road_pixels[0], road_pixels[1]):
            ...
            elif (connected_component_cnt == 1) or (connected_component_cnt > 2): #############changed by xyx for annotating nodes
                status_matrix[i][j] = GraphExtractor.NODE
            # if connected_component_cnt == 2, we think it is a normal internal node
        return status_matrix

    def detect_rn_component(self, status_matrix,district,year):
        ...
        for i, j in zip(node_pixels[0], node_pixels[1]):
            ...
            for node_pixel in node_pixels:
                connected_segment = self.detect_connected_segment(status_matrix, node_pixel,district,year)
    ...
    def detect_connected_segment(self, status_matrix, start_node_pixel,district,year):
        """
    ...
    def construct_undirected_rn(self, nodes, segments, target_path, nb_rows, xscale, yscale, mbr,district,year):
        ...
        rn = nx.DiGraph()
        # if os.path.exists('results_pixel_pred_jingyuxian_2021/coords_list_div_10.txt'):
            # os.remove('results_pixel_pred_jingyuxian_2021/coords_list_div_10.txt')
        # if os.path.exists('results_pixel_bone_pred_lingqiuxian_2021/coord_list_bone2.txt'):
            # os.remove('results_pixel_bone_pred_lingqiuxian_2021/coord_list_bone2.txt')
        ...
        #     eid += 1
            ...
            write_coords_line_to_txt(coords,district,year)
            # endregion

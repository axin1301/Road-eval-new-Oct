...
def concat_all_label_image(year,district):
    ...
    pred_img_list = glob.glob('../temp_output/'+district+'_road_label_by_image_'+str(year)+'/*.png')
    ...
    for k in range(len(y_tile_list)):
        ...
        if not os.path.exists('../temp_output/'+district+'_road_label_by_image_'+str(year)+'/' + img_name  +'.png'):
            continue
        
        img_temp=Image.open('../temp_output/'+district+'_road_label_by_image_'+str(year)+'/' + img_name  +'.png')
    ...
    resized_img.save('../temp_output/'+district+'_GT_primary_'+str(year)+'.png')

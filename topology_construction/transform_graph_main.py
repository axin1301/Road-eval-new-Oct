...
def transform_graph_main(year, district):

    df = pd.read_csv('../data/district_boundary_long_lat3.csv')

    conf_path = '../data/json/test_rn_OSM_full_'+district+'.json'

    if not os.path.exists('../data/tdrive_sample/results_GT_'+district+'_'+str(year)+'/'):
        os.makedirs('../data/tdrive_sample/results_GT_'+district+'_'+str(year)+'/')

    results_path = '../data/tdrive_sample/results_GT_'+district+'_'+str(year)+'/'
    ...
    if phase == 1:
        ...
        skeleton = Image.open('../temp_output/topology_construction/'+latin_name+'_GT_'+str(year)+'.png')
        skeleton = np.array(skeleton)
        map_extractor = GraphExtractor(epsilon=10, min_road_dist=0.001)

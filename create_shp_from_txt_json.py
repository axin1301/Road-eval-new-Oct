...
def create_shp_from_txt_json(district, year):
    ...
    year_list = [year]
    if not os.path.exists('results_pixel_coored_final_shp/'):
        os.makedirs('results_pixel_coored_final_shp/')
    for district in district_list:
        for year in year_list:

            # 示例文件路径
            original_file_path = 'results_pixel_bone_pred/results_pixel_bone_pred_'+district+'_'+str(year)+'_coord/coord_list_simplified_edge.txt' #'output2_GE_jingyuxian.txt'
            transformed_file_path = 'results_pixel_coored_final_wgs/'+district+'_'+str(year)+'-GE-18-final-wgs.json' #'jingyuxian-GE-18-final-wgs.json'
            output_path = 'results_pixel_coored_final_shp/'+district+'_'+str(year)+'-GE-18-final-wgs.shp'
